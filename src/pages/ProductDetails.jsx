...
function ProductDetails() {
  ...
  return (
    <div className="min-h-screen bg-red-700 p-2 sm:p-4 flex flex-col pb-32">
      {/* Header */}
      <motion.div
      ...
      >
        <h1 className="text-2xl sm:text-3xl font-black uppercase text-center mb-1 tracking-tight rotate-1 flex items-center justify-center gap-2">
          <span className="text-2xl sm:text-3xl"><Leaf size={32} className="sm:w-10 sm:h-10 mt-1 sm:mt-2"/></span>
          Product Details
        </h1>
        <p className="text-center text-xs sm:text-sm font-black uppercase tracking-wide">Sustainability Analysis</p>
      </motion.div>
      ...
      >
        ...
        {/* Back Button Section */}
        <div className="p-3 sm:p-6 bg-yellow-300 border-b-2 sm:border-b-4 border-black">
          <button
            onClick={() => navigate('/')}
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-6 pb-32">
          ...
          {productInfo.image && (
            ...
            >
              <img
                ...
                className="w-32 h-32 sm:w-48 sm:h-48 object-cover border-2 sm:border-4 border-black mx-auto -rotate-2"
                style={{ border: '2px solid #000', boxShadow: '4px 4px 0px #000' }}
              />
            </motion.div>
          )}
          ...
          <motion.div
            className="bg-blue-500 border-2 sm:border-4 border-black p-3 sm:p-4 mb-4 sm:mb-6 -rotate-1"
            style={{ boxShadow: '3px 3px 0px #000' }}
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
              <div className="flex-1">
                <h2 className="text-xl sm:text-2xl font-black uppercase text-center sm:text-left tracking-tight">{productInfo.name}</h2>
              </div>
              <button
                onClick={async () => {
                ...
                className="bg-pink-300 border-2 sm:border-4 border-black px-3 sm:px-4 py-2 font-black text-xs sm:text-sm uppercase hover:scale-105 transition-transform rotate-1"
          ...
          {/* Eco-Score */}
          <motion.div
            className="bg-lime-400 border-2 sm:border-4 border-black p-4 sm:p-6 mb-3 sm:mb-4"
            style={{ boxShadow: '3px 3px 0px #000' }}
          ...
          >
            <div className="flex items-center justify-center gap-3 sm:gap-4">
              <span className="text-4xl sm:text-6xl mr-1 sm:mr-2"><Sprout size={36} className="sm:w-14 sm:h-14"/></span>
              <div className="text-center">
                <div className="text-xs sm:text-sm font-black uppercase mb-1">Eco-Score</div>
          ...
          {(() => {
            ...
            return productInfo.ingredientConcerns && productInfo.ingredientConcerns.length > 0 && (
              ...
              >
                <motion.div
                  className="bg-black text-white p-2 sm:p-3 border-2 border-black mb-3 sm:mb-4 rotate-1"
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.4, ease: "easeOut", delay: 0.9 }}
                >
                  <p className="font-black text-xs sm:text-sm uppercase tracking-tight">⚠️ CONCERNING INGREDIENTS DETECTED</p>
                </motion.div>
                {productInfo.ingredientConcerns.map((concern, index) => {
                  ...
                  return (
                    <motion.div
                      key={index}
                      className={`p-3 sm:p-4 mb-2 sm:mb-3 border-2 sm:border-4 ${severityColor} -rotate-1`}
                      style={{ boxShadow: '3px 3px 0px #000', transform: `rotate(${index % 2 === 0 ? -1 : 1}deg)` }}
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ duration: 0.4, ease: "easeOut", delay: 1.0 + (index * 0.1) }}
                    >
                      <div className="flex items-center gap-2 sm:gap-3">
                        <span className="text-3xl sm:text-4xl">{concern.icon}</span>
                        <div>
                          <h3 className="font-black text-base sm:text-lg uppercase text-black tracking-tight">
                             {concern.category.toUpperCase()}
                          </h3>
                          <p className="font-black text-xs sm:text-sm uppercase text-black tracking-tight mt-1">
                            {concern.description}
                          </p>
                          <div className="mt-2">
          ...
          <motion.div
            className="grid grid-cols-2 md:grid-cols-3 gap-2 sm:gap-3 md:gap-4 mb-3 sm:mb-4"
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            {productInfo.packaging_impact && productInfo.packaging_impact.impact !== 'unknown' && (
              <motion.div
                className="bg-lime-400 border-2 sm:border-4 border-black p-2 sm:p-3 col-span-2 md:col-span-1"
                style={{ boxShadow: '3px 3px 0px #000', gridRow: 'span 2' }}
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.4, ease: "easeOut", delay: 1.3 }}
              >
                <h3 className="text-sm sm:text-base font-black uppercase mb-1 sm:mb-2 bg-white border-2 border-black inline-block px-1 py-1 -rotate-2">
                  <Package size={16} className="sm:w-4 sm:h-4" /> Packaging
                </h3>
                <div className={`text-2xl sm:text-3xl md:text-4xl mb-1 font-black ${productInfo.packaging_impact.color === 'green' ? 'text-green-600' : productInfo.packaging_impact.color === 'yellow' ? 'text-yellow-600' : 'text-red-600'}`}>
            ...
            {productInfo.transportation && (
              <motion.div
                className="bg-cyan-400 border-2 sm:border-4 border-black p-2 sm:p-3 col-span-1 md:col-span-1"
                style={{ boxShadow: '3px 3px 0px #000' }}
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.4, ease: "easeOut", delay: 1.4 }}
              >
                <h3 className="text-xs sm:text-sm font-black uppercase mb-1 sm:mb-2 bg-white border-2 border-black inline-block px-1 py-1 rotate-1">
                  <Bus size={14} className="sm:w-4 sm:h-4" /> Transport CO₂
                </h3>
                <div className={`text-lg sm:text-2xl md:text-2xl mb-1 font-black ${productInfo.transportation.co2_kg < 1 ? 'text-green-600' : productInfo.transportation.co2_kg < 3 ? 'text-yellow-600' : 'text-red-600'}`}>
            ...
            {/* Origin */}
            <motion.div
              className="bg-yellow-300 border-2 sm:border-4 border-black p-2 sm:p-3 col-span-1 md:col-span-1"
              style={{ boxShadow: '3px 3px 0px #000' }}
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <h3 className="text-xs sm:text-sm font-black uppercase mb-1 sm:mb-2 bg-white border-2 border-black inline-block px-1 py-1 -rotate-1">
                <Map size={14} className="sm:w-4 sm:h-4" /> Origin
              </h3>
              <div className="text-lg sm:text-2xl md:text-3xl font-black uppercase text-center leading-tight">
          ...
          {/* Ingredients */}
          <motion.div
            className="bg-white border-2 sm:border-4 border-black p-4 sm:p-6 mb-3 sm:mb-4"
            style={{ boxShadow: '3px 3px 0px #000' }}
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, ease: "easeOut", delay: 1.6 }}
          >
            <h3 className="text-lg sm:text-xl font-black uppercase mb-3 sm:mb-4 bg-cyan-400 border-2 border-black inline-block px-2 sm:px-3 py-1">
              <Banana size={18} className="sm:w-5 sm:h-5 inline-block" /> Ingredients
            </h3>
            <p className="text-xs sm:text-sm leading-relaxed font-black uppercase tracking-tight">{productInfo.ingredients}</p>
          </motion.div>
          ...
          {productInfo.nutrientsData && (
            ...
            >
              <h3 className="text-lg sm:text-xl font-black uppercase mb-3 sm:mb-4 bg-white border-2 border-black inline-block px-2 sm:px-3 py-1 -rotate-2">
                <ChartColumn size={18} className="sm:w-5 sm:h-5 inline-block" /> Nutrition (100g)
              </h3>
              <div className="grid grid-cols-2 gap-2 sm:gap-3">
                <div className="bg-white border-2 border-black p-2 sm:p-3 font-black text-xs uppercase">
          ...
          {productInfo.nutrients && Object.keys(productInfo.nutrients).length > 0 && (
            <motion.div
              className="bg-yellow-300 border-2 sm:border-4 border-black p-4 sm:p-6 mb-3 sm:mb-4"
              style={{ boxShadow: '3px 3px 0px #000' }}
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, ease: "easeOut", delay: 1.8 }}
            >
              <h3 className="text-lg sm:text-xl font-black uppercase mb-3 sm:mb-4 bg-white border-2 border-black inline-block px-2 sm:px-3 py-1 rotate-1">
                <Candy size={18} className="sm:w-5 sm:h-5 inline-block" /> Nutrient Levels
              </h3>
              <div className="grid grid-cols-2 gap-2 sm:gap-3">
                {Object.entries(productInfo.nutrients).map(([nutrient, level]) => (
                  level && (
                    <div key={nutrient} className={`border-2 border-black p-2 sm:p-3 ${level === 'low' ? 'bg-green-400' : level === 'moderate' ? 'bg-yellow-400' : 'bg-red-600'}`}>
                      <div className="font-black text-xs uppercase">{nutrient}:</div>
                      <div className="font-black text-base sm:text-lg uppercase">{level}</div>
                    </div>
                  )
                ))}
              </div>
          ...
          {productInfo.mealSuggestions && productInfo.mealSuggestions.length > 0 && !showMealSuggestions && (
            <motion.button
              ...
              className="w-full bg-gray-600 text-white py-3 sm:py-4 px-4 sm:px-6 border-2 sm:border-4 border-black font-black uppercase text-base sm:text-lg tracking-tight transform hover:-translate-y-1 transition-transform mb-3 sm:mb-4"
              style={{ boxShadow: '3px 3px 0px #000' }}
              initial={{ opacity: 0, y: 15 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, ease: "easeOut", delay: 1.9 }}
          ...
          {productInfo.mealSuggestions && productInfo.mealSuggestions.length > 0 && showMealSuggestions && (
            <motion.div
              className="bg-green-400 border-2 sm:border-4 border-black p-4 sm:p-6 m-2 sm:m-4 -rotate-1 relative"
              style={{ boxShadow: '6px 6px 0px #000' }}
              initial={{ opacity: 0, y: 15 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <button
                ...
                className="absolute top-1 sm:top-2 right-1 sm:right-2 bg-red-500 hover:bg-red-600 text-white border-2 sm:border-3 border-black w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center font-black text-sm sm:text-lg rounded-full transition-transform hover:scale-110"
                style={{ boxShadow: '2px 2px 0px #000' }}
              >
                ×
              </button>
              ...
              <motion.h2
                className="text-2xl sm:text-3xl font-black uppercase mb-4 sm:mb-6 bg-white border-2 sm:border-4 border-black inline-block px-3 sm:px-4 py-2 rotate-1"
                style={{ boxShadow: '3px 3px 0px #000' }}
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              <motion.p
                className="mb-3 sm:mb-4 font-black text-xs sm:text-sm uppercase bg-white border-2 border-black p-2 sm:p-3"
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                <ul className="space-y-1 sm:space-y-2">
                  {productInfo.mealSuggestions.map((meal, index) => (
                    <motion.li
                      key={index}
                      className="flex items-center gap-2 sm:gap-3 font-black text-base sm:text-lg uppercase"
